    ...
    const ConformingReductionOptions &options)
{
    ...
    if (n_steps == 0)
    {
        MultipleStepSurfaceReductionResult out;
        out.segments = {ReducedSurfaceLeaf{initial_surface, 0.0, 0, initial_surface.u_domain,
                                           initial_surface.v_domain}};
        out.n_competing = 1;
        out.seconds_compete = 0.0;
        out.seconds_coalesce = 0.0;
        return out;
    }
    ...
    const std::size_t n_competing = cells.size();
    ...
    std::fprintf(stderr, "[timing] grid-build %.2fs  coalesce %.2fs  competing %zu -> leaves %zu\n",
                 std::chrono::duration<double>(t_grid - t_start).count(),
                 std::chrono::duration<double>(t_end - t_grid).count(), n_competing,
                 leaves.size());
#endif
    int over = 0;
    ...
    MultipleStepSurfaceReductionResult out;
    out.segments = std::move(leaves);
    out.n_competing = n_competing;
    out.seconds_compete = std::chrono::duration<double>(t_grid - t_start).count();
    out.seconds_coalesce =
        options.coalesce ? std::chrono::duration<double>(t_end - t_grid).count() : 0.0;
    return out;
}
