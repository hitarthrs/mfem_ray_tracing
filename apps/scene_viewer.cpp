...
//   [ / ]           denser / sparser draw stride (LOD for large scenes)
...
// Optional thin-out via [ / ]; default stride=1 draws every patch.
constexpr int kGeometryBatchQuads = 4096;

std::size_t CountDrawnPatches(std::size_t n_patches, int draw_stride)
{
    if (n_patches == 0 || draw_stride < 1) return 0;
    return (n_patches + static_cast<std::size_t>(draw_stride) - 1) /
           static_cast<std::size_t>(draw_stride);
}
...
UiLayout MakeUiLayout(int /*w*/, int h)
{
    ...
    const int pill_h = 24;
    return {
        SDL_Rect{18, 18, 320, std::min(560, h - 120)},
...
void DrawOverlay(SDL_Renderer *r, const Scene &scene, const std::vector<Ray> &shots,
                 ...
                 const std::string &current_path, int draw_stride)
{
    SDL_SetRenderDrawBlendMode(r, SDL_BLENDMODE_BLEND);
    const UiLayout ui = MakeUiLayout(w, h);
    const std::size_t drawn = CountDrawnPatches(scene.patches.size(), draw_stride);
    ...
    {
        std::ostringstream drawn_line;
        drawn_line << drawn << "/" << scene.patches.size() << " s" << draw_stride;
        DrawMetric(r, 36, 154, "DRAWN", drawn_line.str(), draw_stride > 1);
    }
    DrawMetric(r, 36, 180, "GRID RAYS", std::to_string(scene.rays.size()));
    DrawMetric(r, 36, 206, "GRID HITS", CountHits(scene.rays), true);
    DrawMetric(r, 36, 232, "SHOTS", std::to_string(shots.size()));
    DrawMetric(r, 36, 258, "SHOT HITS", CountHits(shots), true);
    ...
    DrawText(r, 36, ui.recast_grid.y + 72, "[ ] OPTIONAL STRIDE", 1);
    SetColor(r, 11, 122, 110, 220);
    DrawText(r, 36, ui.recast_grid.y + 92, "DROP JSON FILE TO LOAD", 1);
...
void FlushGeometryBatch(SDL_Renderer *renderer, std::vector<SDL_Vertex> &verts,
...
void AppendQuad(std::vector<SDL_Vertex> &verts, std::vector<int> &indices,
                ...
                const ScreenPoint &p3, SDL_Color fill)
{
    const int base = static_cast<int>(verts.size());
    verts.push_back(Vertex(p0, fill));
    verts.push_back(Vertex(p1, fill));
    verts.push_back(Vertex(p2, fill));
    verts.push_back(Vertex(p3, fill));
...
void AppendWireEdge(std::vector<SDL_Vertex> &verts, std::vector<int> &indices,
                    const ScreenPoint &a, const ScreenPoint &b, SDL_Color color, float half_px)
{
    const float dx = static_cast<float>(b.x - a.x);
    const float dy = static_cast<float>(b.y - a.y);
    const float len = std::sqrt(dx * dx + dy * dy);
    if (len < 1e-3f) return;
    const float px = -dy / len * half_px;
    const float py = dx / len * half_px;
    const ScreenPoint p0{a.x - px, a.y - py, a.z};
    const ScreenPoint p1{a.x + px, a.y + py, a.z};
    const ScreenPoint p2{b.x + px, b.y + py, b.z};
    const ScreenPoint p3{b.x - px, b.y - py, b.z};
    AppendQuad(verts, indices, p0, p1, p2, p3, color);
}
...
void DrawScene(SDL_Renderer *renderer, const Scene &scene, const std::vector<Ray> &shots,
               ...
               const std::string &error_message, int toast_alpha, int draw_stride)
{
    ...
    const int stride = std::max(1, draw_stride);
    const Basis basis = CameraBasis(cam);
    // Opaque fills skip painter sort — order doesn't matter and saves O(N log N).
    constexpr Uint8 kFillAlpha = 255;
    const SDL_Color wire_color{40, 62, 52, 160};

    std::vector<SDL_Vertex> verts;
    std::vector<int> indices;
    const std::size_t drawn = CountDrawnPatches(scene.patches.size(), stride);
    const std::size_t batch_cap = static_cast<std::size_t>(kGeometryBatchQuads);
    verts.reserve(std::min(drawn, batch_cap) * 4);
    indices.reserve(std::min(drawn, batch_cap) * 6);

    std::size_t quads_in_batch = 0;
    auto maybe_flush = [&]() {
        if (quads_in_batch >= batch_cap)
        {
            FlushGeometryBatch(renderer, verts, indices);
            quads_in_batch = 0;
        }
    };

    for (std::size_t i = 0; i < scene.patches.size(); ++i)
    {
        if (static_cast<int>(i % static_cast<std::size_t>(stride)) != 0) continue;
        const Patch &patch = scene.patches[i];
        std::array<ScreenPoint, 4> p{};
        for (int c = 0; c < 4; ++c) p[c] = Project(patch.p[c], cam, w, h);
        ...
        if (show_surface)
        {
            ...
            const SDL_Color fill{rb, g, static_cast<Uint8>(std::min(255, g + 6)), kFillAlpha};
            AppendQuad(verts, indices, p[0], p[1], p[2], p[3], fill);
            ++quads_in_batch;
            maybe_flush();
        }
        if (show_wire)
        {
            AppendWireEdge(verts, indices, p[0], p[1], wire_color, 0.55f);
            AppendWireEdge(verts, indices, p[1], p[2], wire_color, 0.55f);
            AppendWireEdge(verts, indices, p[2], p[3], wire_color, 0.55f);
            AppendWireEdge(verts, indices, p[3], p[0], wire_color, 0.55f);
            quads_in_batch += 4;
            maybe_flush();
        }
    }
    ...
    DrawOverlay(renderer, scene, shots, show_surface, show_wire, show_hits, show_normals,
                shoot_mode, w, h, current_path, stride);
...
int main(int argc, char **argv)
{
    ...
    auto try_load = [&](const std::string &path) {
        ...
        std::cout << "scene_viewer loaded '" << path << "'\n"
                  << "  patches: " << scene.patches.size() << "\n"
                  << "  drawn:   " << drawn << " (stride " << draw_stride << ", batched)\n"
                  << "  rays:    " << scene.rays.size() << "\n";
    };
...

    {
        ...
        std::cout << "scene_viewer loaded '" << argv[1] << "'\n"
                  ...
                  << "          [ denser / ] sparser optional stride (default draws all)\n"
    ...
    while (running)
    {
        ...
        while (SDL_PollEvent(&ev))
        {
            ...
            if (ev.type == SDL_KEYDOWN)
            {
                ...
                case SDLK_LEFTBRACKET:
                    draw_stride = std::max(1, draw_stride / 2);
                    dirty = true;
                    break;
                case SDLK_RIGHTBRACKET:
                    draw_stride = std::min(draw_stride * 2, 4096);
                    dirty = true;
                    break;
                case SDLK_c:
                    shots.clear();
        ...
        if (toast_alpha != last_toast_alpha)
        {
            dirty = true;
            last_toast_alpha = toast_alpha;
        }
        ...
        DrawScene(renderer, scene, shots, cam, w, h, show_surface, show_wire, show_hits,
                 show_normals, shoot_mode, current_path, error_message, toast_alpha, draw_stride);
    }
